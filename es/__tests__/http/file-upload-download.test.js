...
describe('HttpClient 文件上传和下载', () => {
    ...
    describe('基本文件上传功能', () => {
        test('upload方法应正确发送FormData', async () => {
            ...
            // 验证请求
            expect(spyPost).toHaveBeenCalledWith('/api/upload', expect.any(FormData), expect.objectContaining({
    ...
    describe('大文件上传功能', () => {
        test('uploadLargeFile方法应分块上传文件', async () => {
            ...
            expect(spyUploadLargeFile).toHaveBeenCalledWith('/api/upload/large', expect.any(File), expect.objectContaining({
                chunkSize: 1024 * 1024
            }));
        });
    });
    describe('文件下载功能', () => {
        test('download方法应下载文件', async () => {
            ...
            // 验证请求
            expect(spyDownload).toHaveBeenCalledWith('/api/download/123', expect.objectContaining({
                responseType: 'blob'
            }));
            // 验证结果是Blob
    ...
    describe('大文件下载功能', () => {
        test('downloadLargeFile方法应先获取文件大小', async () => {
            ...
            // 验证调用
            expect(spyDownloadLargeFile).toHaveBeenCalledWith('/api/download/large/123', expect.objectContaining({
                fileName: 'large-file.dat'
            }));
            // 验证结果是Blob
            expect(result).toBeInstanceOf(Blob);
